...
async function ensureNotesConfiguration(userId: string) {
  ...
  if (!categories.some((category) => category.id === settings.defaultCategoryId)) {
    return {
      categories,
...
notesRoutes.patch("/categories/:id", async (context) => {
  ...
  if (!existing) return context.json({ error: "Note category not found" }, 404);
...
notesRoutes.delete("/categories/:id", async (context) => {
  ...
  const replacementId = categories.find((category) => category.id !== targetId)!.id;
...
notesRoutes.patch("/settings", async (context) => {
  const userId = context.get("session").user.id;
  const parsed = updateNotesSettingsSchema.safeParse(await context.req.json());
  if (!parsed.success) {
    return context.json(
...
notesRoutes.post("/", async (context) => {
  ...
  if (!parsed.success) {
    return context.json({ error: "Invalid note", issues: parsed.error.issues }, 400);
  }
  const category = await prisma.noteCategory.findFirst({
    where: { id: parsed.data.categoryId, userId },
    select: { id: true },
  });
  if (!category) return context.json({ error: "Note category not found" }, 400);
...
notesRoutes.delete("/:id", async (context) => {
  ...
  if (result.count === 0) return context.json({ error: "Note not found" }, 404);
